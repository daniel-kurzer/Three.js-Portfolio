    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" className='flex items-center gap-2 hover:scale-110 transition-all duration-300' onClick={() => { setActive(""); window.scrollTo(0, 0)}}>
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Daniel Kurzer &nbsp;<span className='sm:block hidden'>| DEV</span></p>
        </Link>
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link) => ( 
            <li key={link.id} className={`${active === link.title ? "text-white" : "text-secondary"} hover:text-white text-[18px] font-medium cursor-pointer decoration-purple-600 hover:underline underline-offset-8 hover:scale-125 transition-all duration-300`} onClick={() => setActive(link.title)}>
